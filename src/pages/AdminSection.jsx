...
import { _store, _members, _contents, addSubmission, useStore, setMembers, setContents } from "../utils/store.js";
...
export function AdminSection({ setActive, authed, setAuthed }) {
  ...
  useEffect(() => { setContents(contentsState); saveContents(contentsState); }, [contentsState]);
  ...
  const ContentForm = ({ item, onSave, onCancel }) => {
    ...
    const handleSave = () => {
      if (!f.title) return;
      const savedItem = { ...f, id: item?.id || Date.now(), views: Number(f.views)||0 };
                  ...
                  return [
                    <tr key={c.id||i} style={{ background:isHidden ? "rgba(192,57,43,0.04)" : (i%2===0?"transparent":"rgba(10,22,40,0.015)"), opacity:isHidden?0.6:1 }}>
                      ...
                      <td style={{ ...tdStyle, textAlign:"right", whiteSpace:"nowrap" }}>
                        ...
                        <button onClick={() => setEditContent(c)} style={{ ...btnPrimary, padding:"5px 12px", marginRight:6 }}>수정</button>
                        <button onClick={() => {
