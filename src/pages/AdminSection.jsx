...
export function AdminSection({ setActive, authed, setAuthed }) {
  ...
  const [nlForm, setNlForm] = useState({ title:"", greeting:"안녕하세요, 뷰인사이드입니다.", body:"", closing:"감사합니다.\n\n뷰인사이드\n대표 노무사 김재정\nhwayulinside@gmail.com" });
  ...
  if (!authed) return (
    <section style={{ minHeight:"100vh", background:C.navy, display:"flex", alignItems:"center", justifyContent:"center", padding:32 }}>
      <div style={{ maxWidth:400, width:"100%", textAlign:"center" }}>
        ...
        <p style={{ fontSize:13, color:"rgba(244,241,235,0.4)", marginBottom:12 }}>뷰인사이드 관리자 전용 페이지입니다.</p>
  ...
  const ContentForm = ({ item, onSave, onCancel }) => {
    ...
    const AI_TOPICS = {
      "판례사례": { tag:"판례",     type:"case",     prompt:"최신 직장내 괴롭힘 관련 법원 판례 1건을 선정하여 콘텐츠를 작성하세요. 사건번호, 사실관계, 판결 요지, 실무적 시사점을 포함하세요." },
      "산재사례": { tag:"산재",     type:"sanjae",   prompt:"직장내 괴롭힘·업무 스트레스로 인한 산재 승인/불승인 사례 1건을 선정하여 콘텐츠를 작성하세요. 피해 상황, 인정/불인정 근거, 실무 팁을 포함하세요." },
      "뉴스":     { tag:"뉴스",     type:"news",     prompt:"직장내 괴롭힘·산재 관련 최신 법령 개정·정책·보도자료 1건을 선정하여 작성하세요. 주요 내용, 시행일, 기업·근로자 대응 방안을 포함하세요." },
      "자료":     { tag:"자료",     type:"resource", prompt:"직장내 괴롭힘·산재 관련 실무 자료(서식·가이드·체크리스트)의 제목과 설명을 작성하세요. 활용 대상, 포함 내용, 활용 방법을 포함하세요." },
      "칼럼":     { tag:"칼럼",     type:"column",   prompt:"직장내 괴롭힘·산재 관련 전문 칼럼을 작성하세요. 20년 노무사 현장 경험과 실무적 시사점을 포함하세요." },
    };
    ...
    return (
      <div style={{ ...cardStyle, marginBottom:20 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:12, marginBottom:12 }}>
          <div><label style={{ fontSize:11, fontWeight:700, color:C.gray, display:"block", marginBottom:4 }}>유형</label><select value={f.type} onChange={U("type")} style={inputStyle}><option value="case">⚖️ 판례사례</option><option value="sanjae">🏥 산재사례</option><option value="news">📰 뉴스</option><option value="resource">📎 자료</option><option value="column">✏️ 칼럼</option></select></div>
            ...
            <div style={cardStyle}>
              ...
              {d.length === 0 ? <div style={{ textAlign:"center", padding:48, color:C.gray }}>📭 접수된 내역이 없습니다.</div> : (
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", minWidth:1100 }}>
                  ...
                  <tbody>{d.map((r,i)=>(
                    <tr key={r.id} style={{ background:i%2===0?"transparent":"rgba(10,22,40,0.015)" }}>
                      <td style={{ ...tdStyle, whiteSpace:"nowrap" }}>{new Date(r.submittedAt).toLocaleString("ko-KR")}</td>
                      <td style={{ ...tdStyle, fontWeight:700, whiteSpace:"nowrap" }}>{r.name||"-"}</td>
                      <td style={{ ...tdStyle, whiteSpace:"nowrap" }}>{r.company||"-"}</td>
                      <td style={{ ...tdStyle, whiteSpace:"nowrap" }}>{r.phone||"-"}</td>
                      <td style={{ ...tdStyle, whiteSpace:"nowrap" }}>{r.email||"-"}</td>
                      <td style={{ ...tdStyle, whiteSpace:"nowrap" }}>{r.diagResult ? <button onClick={()=>{const w=window.open("","_blank","width=900,height=700");if(w){const isHtml=r.diagResult.trim().startsWith("<");w.document.write(isHtml?r.diagResult:`<html><body style="font-family:sans-serif;padding:24px;"><h3>진단 결과 데이터</h3><pre style="white-space:pre-wrap;line-height:1.8;">${typeof r.diagResult==="string"?r.diagResult:JSON.stringify(JSON.parse(r.diagResult),null,2)}</pre></body></html>`);w.document.close();}}} style={{ padding:"4px 10px", borderRadius:4, background:"rgba(13,115,119,0.1)", border:"1px solid rgba(13,115,119,0.25)", color:C.teal, fontSize:10, fontWeight:700, cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap" }}>📄 결과보기</button> : <span style={{ fontSize:10, color:C.gray }}>없음</span>}</td>
                      <td style={{ ...tdStyle, whiteSpace:"nowrap" }}>
                        {r.referencedContent?.id ? (
                          <a href={`/content/${r.referencedContent.id}`} target="_blank" rel="noopener noreferrer"
                            ...
                            style={{ padding:"3px 9px", borderRadius:100, background:"rgba(13,115,119,0.1)", color:C.teal, border:"1px solid rgba(13,115,119,0.25)", fontSize:10, fontWeight:700, cursor:"pointer", fontFamily:"inherit", maxWidth:150, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", display:"inline-block", textDecoration:"none", verticalAlign:"middle" }}>
                            📄 {(r.referencedContent.title||"").slice(0,10)}{(r.referencedContent.title||"").length > 10 ? "…" : ""}
                          </a>
            ...
            try {
              const res = await fetch("https://hwayul-backend-production-96cf.up.railway.app/api/claude", {
                ...
                body: JSON.stringify({
                  ...
                  system:`당신은 직장내 괴롭힘·노동법 전문 노무사 사무소(뷰인사이드)의 뉴스레터 편집장입니다.
...
<div class="wrap">
  ...
  <div style="background:#0A1628;padding:20px 32px;text-align:center">
    <div style="font-size:10px;color:rgba(244,241,235,0.3);line-height:1.8">© 2025 뷰인사이드 | hwayulinside@gmail.com<br/><a href="#" style="color:rgba(244,241,235,0.4);font-size:9px">수신 거부</a></div>
  </div>
</div></body></html>`;
              ...
              <div style={cardStyle}>
                {nlMode === "compose" && (<>
                  ...
                  <div style={{ marginBottom:12 }}>
                    ...
                    <input value={nlForm.title} onChange={e=>setNlForm(f=>({...f,title:e.target.value}))} placeholder="예: [뷰인사이드] 2026년 3월 직장내 괴롭힘 최신 판례 정리" style={{ width:"100%", padding:"11px 13px", borderRadius:8, border:`2px solid ${nlForm.title?"rgba(13,115,119,0.3)":"rgba(10,22,40,0.1)"}`, fontSize:13, fontFamily:"inherit", outline:"none", boxSizing:"border-box" }} />
                  </div>
