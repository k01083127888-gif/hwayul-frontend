...
//
// 발견된 깨진 코드포인트:
//   U+F914 = "樂" (CJK Compatibility Ideograph) → 원래 🤔
//   U+F4A1 (Private Use Area)                  → 원래 💡
// Private Use Area(U+E000-U+F8FF) + CJK Compatibility 영역에서 깨진 글자가
// 헤딩 앞에 자주 옴. 헤딩 안에서만 보수적으로 교정.
const fixBrokenEmojis = (html) => (html || "")
  // U+F914 (樂)이 헤딩 시작에 단독으로 있으면 → 🤔
  .replace(/<(h[2-4])>\s*[樂]\s*/g, "<$1>🤔 ")
  // Private Use Area 한 글자(U+E000-U+F8FF) + 일반 PUA 한자가 헤딩 시작에 있으면 → 💡
  // (대부분 💡 같은 이모지가 깨진 흔적)
  .replace(/<(h[2-4])>\s*[-]\s*/g, "<$1>💡 ")
  // 옛날 fallback: "樂" 일반 한자도 처리 (위 규칙 안 맞을 때)
  .replace(/<(h[2-4])>\s*樂\s*/g, "<$1>🤔 ")
