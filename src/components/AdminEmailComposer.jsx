        ...
        <div style={{ display:"flex", gap:8 }}>
          ...
          <button onClick={async () => {
            ...
            if (data.to) {
              try {
                const subjectMap = {
                  "report":"[WIHAM 인사이드] 익명 제보 검토 결과 안내",
                  "biz":"[WIHAM 인사이드] 심층 상담 회신",
                  "relief":"[WIHAM 인사이드] 해결 의뢰 검토 결과 안내",
                  "review":"[WIHAM 인사이드] 리포트 검토 회신",
                  "report-email":"[WIHAM 인사이드] 진단 결과지 발송",
                  "consulting":"[WIHAM 인사이드] 상담 안내",
                  "lecture":"[WIHAM 인사이드] 강의 요청 회신",
                  "advisory":"[WIHAM 인사이드] 자문 요청 회신",
                };
                const sendRes = await fetch("https://hwayul-backend-production-96cf.up.railway.app/api/send-email", {
                  ...
                  body: JSON.stringify({
                    to: data.to,
                    subject: subjectMap[data.type] || "[WIHAM 인사이드] 안내 메일",
                    html: emailHtml
                  })
                });
