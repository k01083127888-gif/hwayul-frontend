...
반드시 [섹션1] ~ [섹션6] 태그로 시작하세요. 태그 외에 제목이나 번호를 붙이지 마세요.`;
  ...
  // 리포트를 HTML로 변환
  const buildReportHtml = () => `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${diagType} 검토 리포트 - 뷰인사이드</title>
...
<div class="footer">
뷰인사이드 | hwayulinside@gmail.com | 02-2088-1767<br/>
...
<div class="wrap">
  ...
  <div style="background:#0A1628;padding:20px 32px;text-align:center">
    <div style="font-size:10px;color:rgba(244,241,235,0.3);line-height:1.8">&copy; 2025 뷰인사이드 | Tel. 02-2088-1767 | hwayulinside@gmail.com</div>
  </div>
</div></body></html>`;
    ...
    if (r.email) {
      try {
        await fetch("https://hwayul-backend-production-96cf.up.railway.app/api/send-email", {
          ...
          body: JSON.stringify({
            to: r.email,
            subject: `[뷰인사이드] ${diagType} 노무사 검토 리포트`,
      ...
      try {
        await fetch("https://hwayul-backend-production-96cf.up.railway.app/api/sent-emails", {
          ...
          body: JSON.stringify({
            ...
            greeting: emailBody || `${r.name||""} 님 안녕하세요, 뷰인사이드입니다.`,
            body: "검토 리포트 첨부 발송",
            closing: "감사합니다. 뷰인사이드 대표 노무사 김재정",
