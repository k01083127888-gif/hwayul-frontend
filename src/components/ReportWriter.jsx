...
반드시 [섹션1] ~ [섹션6] 태그로 시작하세요. 태그 외에 제목이나 번호를 붙이지 마세요.`;
  ...
  // 리포트를 HTML로 변환
  const buildReportHtml = () => `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${diagType} 검토 리포트 - 화율인사이드</title>
...
&copy; ${new Date().getFullYear()} 화율랩스. All rights reserved.
</div></body></html>`;
  ...
  const downloadReport = () => {
    ...
    a.download = `화율_검토리포트_${r.orgName||r.name||""}_${new Date().toISOString().slice(0,10)}.html`;
...
<div class="wrap">
  <div style="background:#0A1628;padding:28px 32px;text-align:center">
    <div style="font-size:20px;font-weight:800"><span style="color:#E8E5DE">화율</span> <span style="color:#4ECDC4">인사이드</span></div>
  ...
  <div style="background:#0A1628;padding:20px 32px;text-align:center">
    <div style="font-size:10px;color:rgba(244,241,235,0.3);line-height:1.8">&copy; 2025 화율인사이드 | Tel. 02-2088-1767 | hwayulinside@gmail.com</div>
  </div>
</div></body></html>`;
    ...
    if (r.email) {
      try {
        await fetch("https://hwayul-backend-production-96cf.up.railway.app/api/send-email", {
          ...
          body: JSON.stringify({
            to: r.email,
            subject: `[화율인사이드] ${diagType} 노무사 검토 리포트`,
  ...
  if (emailMode) return (
    <div style={overlay} onClick={onClose}><div style={modal} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{ padding:"10px 14px", background:"rgba(13,115,119,0.06)", borderRadius:8, marginBottom:16, fontSize:11, color:C.gray }}>
        📎 <b>첨부파일:</b> 화율_검토리포트_{r.orgName||r.name||""}_{new Date().toISOString().slice(0,10)}.html
      </div>
